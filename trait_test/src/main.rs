...
fn main() {
    ...
    let R_var = f32::cos(PI / 4.0);

    let material_left = Lambretian::new(Vector3::new(0.0, 0.0, 1.0));
    let material_right = Lambretian::new(Vector3::new(1.0, 0.0, 0.0));
    let left_s = Sphere::new(
        Vector3::new(-R_var, 0.0, -1.0),
        R_var,
        Box::new(material_left),
    );
    let right_s = Sphere::new(
        Vector3::new(R_var, 0.0, -1.0),
        R_var,
    ...
    w.add_object(Box::new(left_s));
    w.add_object(Box::new(right_s));
